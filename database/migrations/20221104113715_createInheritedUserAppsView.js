...
exports.up = function (knex) {
  return knex.schema.raw(`
    ...
    AS
      SELECT user_id, jsonb_agg(inherited_apps_ids) AS inherited_apps_ids FROM (
        SELECT DISTINCT(user_id), jsonb_array_elements(inherited_apps_ids) AS inherited_apps_ids FROM (
          SELECT u.id AS user_id, ug.group_id, u.apps_ids AS user_apps_ids, iga.inherited_apps_ids AS group_apps_ids,
          CASE 
